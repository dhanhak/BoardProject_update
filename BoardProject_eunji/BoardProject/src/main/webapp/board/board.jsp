...
<body>
	<form action="/search.board" method="get">
		...
		<table border="1" width="100%">
						...
						<c:choose>
							<c:when test="${search != null}">
								<c:choose>
									<c:when test="${item eq '<'}">
										<a
											href="/search.board?cpage=${navi[status.index+1]-1}&sel=${sel}&search=${search}">${item}</a>
									</c:when>
									<c:when test="${item eq '>'}">
										<a
											href="/search.board?cpage=${navi[status.index-1]+1}&sel=${sel}&search=${search}">${item}</a>
									...
									<c:otherwise>
										<a href="/search.board?cpage=${item}&sel=${sel}&search=${search}">${item}</a>
									</c:otherwise>
								</c:choose>
							</c:when>
						</c:choose>
			...
			<tr height="20">
				<td colspan="1"><select name="sel">
						<option value="seq">글번호</option>
						<option value="title">제목</option>
						<option value="writer">작성자</option>
				</select></td>
				<td colspan="2" align="center"><input type="text" name="search"
					style="width: 300px"></td>
				<td align="right" colspan="2"><input type="submit" value="검색">
					<a href="/toIndex.board"> <input type="button" id="back"
						value="뒤로가기"></a> <a href="/toWriteForm.board?cpage=${cpage}">
